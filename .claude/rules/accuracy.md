...
- **No intermediate rounding.** All per-question math (consolidation, normalization scaling) must stay full-precision. Round only once at the very end when writing final `total_earned` and per-question display values. Intermediate `round()` calls on per-question scores accumulate systematic drift that costs every student points. If you must redistribute a remainder across questions (e.g. to make max_points sum to rubric total), apply the same adjustment to earned_points for any question where the student earned full marks.
